...
export class InvoiceComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getData.getFastLog().subscribe(
      (res: any) => {
        if (res.status == 200) {
          this.logArray = res;
        ...
        if (
          this.allPO.find((po) => {
            if (po.status == res) {
              ...
              this.selectedPO = po;
              this.postatus =
                this.selectedPO.invoice !== 'Pending'
                  ? 'Al gestuurd/Déjà envoyé'
                  : 'Nog niet behandeld/Pas encore traité';
            }
            return po === res;
          })
        ) {
  ...
  private _filter3(value: string): string[] {
    ...
    return this.options3.filter((option) =>
      option.toString().includes(filterValue)
    );
  }
            ...
            if (reader.result !== null) {
              ...
              this.sendVendors.sendInvoice(this.fd).subscribe((res) => {
                ...
                if (this.res.response === '250 Message received') {
                  alert(
                    `Invoice naar AP gestuurd! This invoice has been scanned and it passed our tests. It looks legitimate and contains a PO (detected here in green). Thank you for using SBDForms. No further action is required. You may now leave this page.`
                  );
                  this.selected_files = [];
                  this.u_ID = '';
  ...
  getRandomPosition(max: number, initialPos: number) {
    ...
    if (Math.random() < 0.5) {
      ...
      newPosition =
        initialPos +
        Math.floor(Math.random() * (maxDifference - minDifference + 1)) +
        minDifference;
    } else {
      newPosition =
        initialPos -
